...
Luego de responder todas las preguntas del laboratorio me gustaria analizar profundamente el codigo de forma clara y deofuscado. Para eso lo primero que hay que hacer es pasar la extension del archivo de `.xls` a `.vba` para anlizarlo en **Visual Studio Code**. Hay que utilizar el comando: `olevba Siparis_17.xls > Siparis.vba`. Luego para deofuscar el codigo y que sea totalmente legible hay que utilizar el siguiente comando: `olevba --deobf --reveal Siparis.vba > Siparis_deofuscado.vba`. Luego en **Visual Studio** abrimos el archivo `Siparis_deofuscado.vba`.
